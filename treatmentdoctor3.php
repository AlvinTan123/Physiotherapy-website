...
// Handle form submission for adding/editing the assessment
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['submit_tissue'])) {
        $tissue_types = $_POST['tissue_type'] ?? [];
        $tissue_findings = $_POST['tissue_findings'] ?? [];
        $remarks = $_POST['remarks'] ?? [];

        foreach ($tissue_types as $index => $tissue_type) {
            $finding = $tissue_findings[$index];
            $remark = $remarks[$index];

            // Check if record already exists for the same patient and tissue type
            $sql_check = "SELECT * FROM treatmentassessment3 WHERE patient_id = ? AND appointment_id = ? AND tissue_type = ?";
            if ($stmt_check = $conn->prepare($sql_check)) {
                $stmt_check->bind_param("iis", $patient_id, $appointment_id, $tissue_type);
                $stmt_check->execute();
                $result_check = $stmt_check->get_result();

                if ($result_check->num_rows > 0) {
                    // If exists, update the record
                    $sql_update = "UPDATE treatmentassessment3 SET tissue_findings = ?, remarks = ? WHERE patient_id = ? AND appointment_id = ? AND tissue_type = ?";
                    if ($stmt_update = $conn->prepare($sql_update)) {
                        $stmt_update->bind_param("ssiis", $finding, $remark, $patient_id, $appointment_id, $tissue_type);
                        $stmt_update->execute();
                        $stmt_update->close();
                    }
                } else {
                    // Insert new record
                    $sql_insert = "INSERT INTO treatmentassessment3 (appointment_id, patient_id, name, tissue_type, tissue_findings, remarks) VALUES (?, ?, ?, ?, ?, ?)";
                    if ($stmt_insert = $conn->prepare($sql_insert)) {
                        $stmt_insert->bind_param("iissss", $appointment_id, $patient_id, $full_name, $tissue_type, $finding, $remark);
                        $stmt_insert->execute();
                        $stmt_insert->close();
                    }
                }
        ...
        echo "<script>alert('Tissue data successfully saved.'); window.location.href='view_treatment.php';</script>";
    }
}
